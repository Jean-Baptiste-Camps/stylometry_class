...
## Load and clean texts - only if you are working from raw txt
...
```{r}
library("tm")
corpus = VCorpus(DirSource("corpus/raw/", encoding = "UTF-8"), readerControl = list(language = "fre"))
```
...
```{r}
samples = VCorpus(DirSource("corpus/samples_select/", encoding = "UTF-8"), readerControl = list(language = "fre"))
...
## Alternative: loading existing data

```{r}
# Choose the type of data you want to use. Here, words
data = as.matrix(
  read.csv(file="datasets/control-words.csv", sep = ";", header = TRUE, row.names=1, quote = '\"'))[, -1]
# And clean a bit
suspectedProperNames = scan("suspected_proper_names.txt", what = "character", sep =",")
data = data[!rownames(data) %in% suspectedProperNames,]

```

Defining a subgroup of text if necessary (skip otherwise)
...
```{r}
maPAM = cluster::pam(t(data), 6, metric = "manhattan")
maPAM$clustering
maPAM$silinfo
```
...
monSCAN = fpc::dbscan(dist(t(data), method="manhattan"), method="dist", eps = 8, MinPts = 2, showplot=TRUE)
